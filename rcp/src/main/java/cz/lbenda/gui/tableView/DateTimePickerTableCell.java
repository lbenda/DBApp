...
public class DateTimePickerTableCell<S, T> extends TableCell<S, T> {
  ...
  public static <S> Callback<TableColumn<S, LocalDateTime>, TableCell<S, LocalDateTime>> forTableColumn() {
    return var2 -> new DateTimePickerTableCell(null, null);
  }
  ...
  public DateTimePickerTableCell(Callback<Integer, ObservableValue<LocalDateTime>> selectedStateCallback, StringConverter<T> converter) {
    ...
    this.localDatePicker = new LocalDateTimeTextField();
    // this.localDatePicker.setConverter(defaultStringConverter); // FIXME
    this.localDatePicker.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
      switch (event.getCode()) {
        case ENTER:
        case TAB:
          LocalDateTime ld = localDatePicker.getDisplayedLocalDateTime();
  ...
  public void startEdit() {
    if (this.isEditable() && this.getTableView().isEditable() && this.getTableColumn().isEditable()) {
      super.startEdit();
      if (this.isEditing()) {
        this.localDatePicker.setLocalDateTime((LocalDateTime) getItem());
        this.setGraphic(this.localDatePicker);
        this.localDatePicker.requestFocus();
        this.setText(null);
      }
    }
  }
